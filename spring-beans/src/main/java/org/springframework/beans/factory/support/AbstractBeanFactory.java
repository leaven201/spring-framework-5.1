...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		/**
		 * 1.转换对应beanName
		 * 将name转换为准确的beanName，比如取出FactoryBean的前缀修饰符，别名转为最终指的beanName
		 */
		...
		if (sharedInstance != null && args == null) {
			...
			// 原型模式时，如果存在循环依赖例如，A中有B的属性，B中有A的属性
			// 则在创建A时，由于需要创建B，而B中又有A的依赖，因此又会创建造成循环依赖
			// 此时就会出现两个A同时在创建的情况，则需要抛出异常
			if (isPrototypeCurrentlyInCreation(beanName)) {
				throw new BeanCurrentlyInCreationException(beanName);
			}

			// 检测当前的beanDefinition是否已经存在
			...
			try {
				// 合并bean，返回合并后的RootBeanDefinition，如果当前的bean是ChildBeanDefinition，则需要遍历其parent进行合并
				final RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
				// 检测是否为抽象bean，是的话则抛异常BeanIsAbstractException
	...
	/**
	 * 返回合并后的RootBeanDefinition，如果当前的bean是ChildBeanDefinition，则需要遍历其parent进行合并
